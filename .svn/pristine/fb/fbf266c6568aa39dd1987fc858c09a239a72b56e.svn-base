...
public class UserDao extends AbstractDao<UserData> {
	...
	public void removeItem(Long id) {
		/*try {
			UserData user = (UserData) session.get(UserData.class, id);
			session.delete(user);
		} catch (xception e) {
	...
	public UserData login(String email,String password){
		...
		try{
			em=EMFactory.createEntityManager();
			Query query=em.createQuery("select p from "+ entityClass.getName()+" p where p.email=:login and p.password=:pass");
